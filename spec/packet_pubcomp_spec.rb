...
describe PahoMqtt::Packet::Pubcomp do
  context "Create a simple pubcomp packet" do
    it "Successfully create pubcomp packet from header" do
      packet = PahoMqtt::Packet::Base.create_from_header(0x70)
      expect(packet.inspect).to eq("#<PahoMqtt::Packet::Pubcomp: 0x00>")
      expect(packet.parse_body("")).to be nil
    end
    it "Encode packet id" do
      packet = PahoMqtt::Packet::Pubcomp.new({id: 4})
  ...
  context "Raising execption" do
    it "Fail for payload in pubcomp packet" do
      ...
      expect { packet.parse_body("\x01\x00\x00") }.to raise_error(PahoMqtt::PacketFormatException, "Extra bytes at end of Publish Complete packet")
    end
  end
end
